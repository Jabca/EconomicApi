...
def test_transform_yearly_prices(raw_daily, yearly_prices, test_day):
    res = get_yearly_prices(raw_daily, test_day=test_day, depth=4)
...
@pytest.mark.data_transform_op
def test_yearly_prices_from_array(daily_prices, yearly_prices, test_day):
    res = get_yearly_prices_from_array(daily_prices, depth=3, test_day=test_day)
    exp_res = yearly_prices
    compare_dictionaries(res, exp_res)
